...
from ..nets import policy_map, q_critic_map, model_map


class MBPOMemory(Memory):
...
class MBPO(pl.LightningModule):
    ...
    def __init__(self, 
        ...
        **kwargs
        ...
        self._memory = MBPOMemory(
            gamma=self.hparams.gamma, 
            env=gym.make(env, render_mode="human"),
            capacity=self.hparams.buffer_capacity, 
            epoch_size=self._epoch_size)
        self._model = model_map[model_net](**self.env_info) # TODO: make it ensemble
        self._pi = policy_map[pi_net](**self.env_info)
        self._q1 = q_critic_map[q_net](**self.env_info)
        self._q2 = q_critic_map[q_net](**self.env_info)
        self._q1_target = q_critic_map[q_net](**self.env_info).eval()
        self._q2_target = q_critic_map[q_net](**self.env_info).eval()
    ...
    def training_step(self, batch, batch_index):
        ...
        with th.no_grad():
            ...
            next_action = next_action_distr.rsample()
