...
class COQL(pl.LightningModule):
    ...
    def training_step(self, batch, batch_index):
        ...
        information_offpolicy_action = - action_distr.log_prob(action) # rollout_per_step, batch
        if self._piv.independent_actions:
            information_offpolicy_action = information_offpolicy_action.sum(dim=-1)
        with th.no_grad():
            _, next_value_offpolicy = self._piv_target(next_observation) 
        value_offpolicy_rhs = self.hparams.alpha * information_offpolicy_action + reward_target + self.hparams.gamma * next_value_offpolicy * done.logical_not()
        offpolicy_loss = th.nn.functional.mse_loss(value, value_offpolicy_rhs) # only policy parameters are updated by this loss
        ...
        onpolicy_loss = th.nn.functional.mse_loss(value.unsqueeze(0), value_onpolicy_samples_rhs) # only policy parameters are updated by this loss
        # onpolicy_loss = th.nn.functional.mse_loss(value, value_onpolicy_samples_rhs.mean(dim=0)) # only policy parameters are updated by this loss
        # losses
        policy_loss = (self.hparams.beta * offpolicy_loss + self.hparams.alpha * onpolicy_loss ) / (self.hparams.beta + self.hparams.alpha)
